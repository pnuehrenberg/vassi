...
#     *,
#     keypoints_focal: Sequence[int],
#     keypoints_other: Sequence[int],
# ) -> tuple[np.ndarray, np.ndarray]:
#     """
#     Computes the proximitry matrix for a group of individuals.

#     If multiple keypoints are provided (either for the focal or other individual, or both),
#     proximitry is computed as the average distance between all pairwise combinations of focal and other keypoints.

#     Parameters:
#         group: The group of individuals.
#         keypoints_focal: The keypoints to compute the proximitry matrix for the focal individual.
#         keypoints_other: The keypoints to compute the proximitry matrix for the other individuals.
...
#     step = steps.pop()
#     first: float = min(
#         [trajectory.timestamps.min() for trajectory in group.trajectories.values()]
#     )
#     last: float = max(
#         [trajectory.timestamps.max() for trajectory in group.trajectories.values()]
#     )
...
#     for identity, trajectory in group.trajectories.items():
#         for identity_other, trajectory_other in group.trajectories.items():
#             if identity == identity_other:
#                 continue
#             u = group.identifiers.index(identity)
#             v = group.identifiers.index(identity_other)
...
#                 keypoints_1=keypoints_focal,
#                 keypoints_2=keypoints_other,
#                 flat=True,
#             ).min(axis=1)
...
#     *,
#     neighbor_rank: int,
#     proximitry_keypoints_focal: Sequence[int],
#     proximitry_keypoints_other: Sequence[int],
# ) -> AnnotatedGroup:
#     proximitry_matrix, timestamps = get_proximitry_matrix(
#         group,
#         keypoints_focal=proximitry_keypoints_focal,
#         keypoints_other=proximitry_keypoints_other,
#     )
...
#         ["recipient", "start"], ignore_index=True
#     )
...
#             proximitry_keypoints_focal=proximitry_keypoints_focal,
#             proximitry_keypoints_other=proximitry_keypoints_other,
#         )
...
#     See Also:
#         :func:`get_proximitry_matrix` on how to compute the proximitry matrix that determines the neighbors.

#     Note:
#         The keypoints to compute the proximitry matrix (keyword arguments) currently default to the first on the actor, and the first three on the recipient.
#     """
#     if isinstance(sampleable, AnnotatedDataset):
#         return _permute_recipients(
#             sampleable,
#             neighbor_rank=neighbor_rank,
#             proximitry_keypoints_focal=proximitry_keypoints_focal,
#             proximitry_keypoints_other=proximitry_keypoints_other,
...
#         return _permute_recipients_in_group(
#             sampleable,
#             neighbor_rank=neighbor_rank,
#             proximitry_keypoints_focal=proximitry_keypoints_focal,
#             proximitry_keypoints_other=proximitry_keypoints_other,
