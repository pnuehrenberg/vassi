...
#         background_category: str = self.background_category  # type: ignore  # see above
...
#             y_true = encoding_function(
#                 annotations["category"].to_numpy(),
#                 background_category=background_category,
#             )
#             y_pred = encoding_function(
#                 annotations["predicted_category"].to_numpy(),
#                 background_category=background_category,
#             )
#         elif on == "prediction":
#             predictions: pd.DataFrame = self.predictions  # type: ignore
#             y_true = encoding_function(
#                 predictions["true_category"].to_numpy(),
#                 background_category=background_category,
#             )
#             y_pred = encoding_function(
...
#     threshold: bool,
#     decision_thresholds: Optional[Mapping[str, float]],
...
#     ) -> Self:
#         try:
#             default_decision_idx = self.categories.index(default_decision)
#         except ValueError:
...
#             return self
#         probabilities_thresholded = probabilities.copy()
#         for category, threshold in decision_thresholds.items():
#             try:
#                 idx = self.categories.index(category)
#             except ValueError:
#                 raise ValueError(
#                     f"undefined category {category} not supported for thresholding"
#                 )
#             # if below threshold, set to 0
#             probabilities_thresholded[:, idx] = np.where(
#                 probabilities[:, idx] >= threshold, probabilities[:, idx], 0
#             )
#         # if no category surpasses threshold, force background category
#         probabilities_thresholded[
#             probabilities_thresholded.sum(axis=1) == 0,
#             default_decision_idx,
#         ] = 1 if default_decision_idx is not None else 0
#         self.y_pred_numeric = np.argmax(probabilities_thresholded, axis=1)
#         # use -1 for background category if not included in categories
#         self.y_pred_numeric[(probabilities_thresholded == 0).all(axis=1)] = -1
#         return self

#     def threshold(
#         self,
#         decision_thresholds: Optional[Mapping[str, float]] = None,
#         *,
#         default_decision: str,
#     ) -> "ClassificationResult":
#         """
...
#         """
#         self._apply_thresholds(decision_thresholds, default_decision=default_decision)
#         probabilities = self.y_proba
#         try:
#             probabilities = self.y_proba_smoothed
#         except ValueError:
...
#                 index_columns=(),
#             )
#             for dyad, classification_result in self.classification_results.items():
#                 if TYPE_CHECKING:
#                     assert isinstance(dyad, tuple)
