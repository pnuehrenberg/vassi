...
def get_pose_segments(trajectory, keypoint_indices):
    ...
    pose_segments = pose_segments.reshape(len(trajectory['pose']),
                                          -1,
                                          trajectory['pose'].shape[-1])
    return pose_segments


@smooth_output(force_positive=True)
@filter_output
def get_pose_speed(trajectory, keypoint_indices):
    assert len(trajectory['pose']), \
        'miminum trajectory length of two instances required'
    keypoint_indices = np.asarray(keypoint_indices)
...
def get_orientation_vectors(trajectory, keypoint_indices):
    ...
    return calculate_unit_vectors(pose_segments)


@smooth_output(median_filter_window_size=11, savgol_filter_window_size=11)
@filter_output(window_size=11)
def get_pose_segment_lengths(trajectory, keypoint_indices):
    return calculate_element_wise_magnitude(
        get_pose_segments(trajectory, keypoint_indices))


def get_noise(trajectory, window_size=11):
    noise = get_sliding_cumulative_distance(trajectory['pose'], window_size) \
            / get_sliding_distance(trajectory['pose'], window_size)
    return noise
